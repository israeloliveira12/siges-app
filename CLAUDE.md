...
- **`system_settings` e `loan_rate_reference` exigem `auth.uid() is not null` para SELECT** (corrigido na migration_010) — antes qualquer visitante não-autenticado conseguia ler taxas internas/thresholds direto pela API REST do Supabase (a anon key é pública). Se precisar expor mais uma tabela de configuração, nunca use `using (true)` sem pensar synchronously se aquilo devia ser público.
