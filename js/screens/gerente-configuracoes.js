...
// Módulo-level (sobrevive a repaints) — qual aba está ativa. Trocar de aba é
// só um toggle de classe CSS (ver setCfgTab), nunca um refetch/re-render
// completo, pra não perder o que o gerente já tinha digitado num campo.
let cfgActiveTab = 'empresa'; // 'empresa' | 'backup' | 'risco'
...
async function renderGerenteConfiguracoes() {
  ...
  const setCfgTab = (tab) => {
    ...
    document.getElementById('cfg-panel-empresa').classList.toggle('hidden', tab !== 'empresa');
    document.getElementById('cfg-panel-backup').classList.toggle('hidden', tab !== 'backup');
    const panelRisco = document.getElementById('cfg-panel-risco');
    if (panelRisco) panelRisco.classList.toggle('hidden', tab !== 'risco');
  };
  document.getElementById('cfg-tab-empresa').onclick = () => setCfgTab('empresa');
